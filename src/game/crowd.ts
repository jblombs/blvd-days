...
const geoTorso = new THREE.CapsuleGeometry(0.25, 0.4, 2, 5);
const geoHead = new THREE.SphereGeometry(0.22, 8, 6);
const geoLeg = new THREE.CapsuleGeometry(0.09, 0.32, 2, 4);
const geoHair = new THREE.SphereGeometry(0.23, 6, 5);
const geoHood = new THREE.SphereGeometry(0.28, 8, 6);
const geoDrape = new THREE.CapsuleGeometry(0.2, 0.35, 2, 5);
...
export class Crowd {
  ...
  constructor(
    ...
    for (let i = 0; i < count; i++) {
      ...
      const spd = 1.15 + Math.random() * 1.5;
      this.agents.push({
        mesh,
        x,
        z,
  ...
  update(dt: number, colliders: Collider[], playerX: number, playerZ: number, playerR: number) {
    ...
    for (const a of this.agents) {
      ...
      if (a.changeT <= 0) {
        ...
        const ang = Math.random() * Math.PI * 2;
        const spd = 1.05 + Math.random() * 1.55;
...
function makePedestrian(p: Persona): THREE.Group {
  ...
  const torso = new THREE.Mesh(geoTorso, shirtMat);
  torso.position.y = 1.0;
  // No ped shadows — keeps Safari cooler with denser crowds

  const head = new THREE.Mesh(geoHead, skinMat);
  head.position.y = 1.55;

  const legL = new THREE.Mesh(geoLeg, pantsMat);
  legL.position.set(-0.1, 0.38, 0);
  const legR = new THREE.Mesh(geoLeg, pantsMat);
  ...
  if (p.headwear === 'hijab') {
    ...
    const hair = new THREE.Mesh(geoHair, hairMat);
